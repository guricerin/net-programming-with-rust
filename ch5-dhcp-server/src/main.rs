...
fn main() -> Result<()> {
    ...
    let server_socket = UdpSocket::bind("0.0.0.0:67").with_context(|| "Failed to bind socket")?;
    server_socket.set_broadcast(true)?;

    // すべてのスレッドから共有されるのでArcで囲む。
    let dhcp_server = Arc::new(DhcpServer::new().with_context(|| "Failed to start dhcp server.")?);
        ...
        match server_socket.recv_from(&mut recv_buf) {
            Ok((size, src)) => {
                ...
                let transmission_socket = server_socket
                    .try_clone()
                    .with_context(|| "Failed to create client socket")?;
 ...
 */
fn dhcp_handler(packet: &DhcpPacket, soc: &UdpSocket, dhcp_server: Arc<DhcpServer>) -> Result<()> {
    let message = packet
        .get_option(Code::MessageType as u8)
        .with_context(|| "specified option was not found")?;
...
/**
 * DHCPREQUESTメッセージのオプションにserver_identifierが含まれる場合のハンドラ。
...
fn dhcp_request_message_handler_responded_to_offer(
    ...
    let server_ip =
        util::u8_to_ipv4addr(&server_id).with_context(|| "Failed to convert ip addr.")?;
    ...
    let ip_to_be_leased =
        util::u8_to_ipv4addr(&ip_bin).with_context(|| "Failed to convert ip addr.")?;

    let mut conn = dhcp_server.db_connection.lock().unwrap();
    let count = {
        // クリティカルセクションを短くするため、ブロックに入れる
        let tx = conn.transaction()?;
        let count = database::count_records_by_macaddr(&tx, client_macaddr)?;
        match count {
            // レコードがないなら追加。
...
fn dhcp_request_message_handler_to_reallocate(
    ...
    if let Some(requested_ip) = received_packet.get_option(Code::RequestedIpAddress as u8) {
        // クライアントが以前割り当てられたIPアドレスを記憶していて、再起動状態にあるとき
        debug!("client is in INIT_REBOOT");
        let requested_ip =
            util::u8_to_ipv4addr(&requested_ip).with_context(|| "Failed to conert ip addr.")?;
        let conn = dhcp_server.db_connection.lock().unwrap();
        match database::select_entry(&conn, client_macaddr)? {
            Some(ip) => {
                if ip == requested_ip && dhcp_server.network_addr.contains(ip) {
                    // 以前割り当てたIPアドレスと要求されたIPアドレスが一致しており、ネットワークに含まれる場合はACKを返す。
                    let dhcp_packet =
            ...
            None => {
                // レコードがないならなにもしてはいけない（RFC2131 p.32）
                Ok(())
            }
        }
    } else {
        ...
        if !dhcp_server.network_addr.contains(ip_from_client) {
            return Err(anyhow!("Invalid ciaddr. Mismatched network address."));
        }

        let dhcp_packet =
            make_dhcp_packet(&received_packet, &dhcp_server, DHCPACK, ip_from_client)?;
        util::send_dhcp_broadcast_response(soc, dhcp_packet.get_buffer())?;
        info!("{:x}: sent DHCPACK", xid);
        Ok(())
    }
...
fn select_lease_ip(
...
) -> Result<Ipv4Addr> {
    {
        ...
        if let Some(ip_from_used) = database::select_entry(&conn, received_packet.get_chaddr())? {
            // IPアドレスが重複していないか
    ...
    // Requested Ip Addrオプションがあり、利用可能ならばそのIPアドレスを返す
    ...
    Err(anyhow!("Could not obtain available ip address."))
}
...
fn obtain_available_ip_from_requested_option(
    ...
    let ip = util::u8_to_ipv4addr(&ip)?;
    // アドレスプールから検索
    let ip = dhcp_server.pick_specified_ip(ip)?;
    if util::is_ipaddr_available(ip).is_ok() {
        Some(ip)
    } else {
        None
    }
...
fn make_dhcp_packet(
    ...
    let mut dhcp_packet = DhcpPacket::new(buf).context("failed to make a DhcpPacket.")?;
